  ...
  return (
    ...
    >
      <div className="mb-[4vh]">
        <h3 className={`${inter.className} ${styles.name}`}>
          {name}
        </h3>
        <p className={`${inter.className} ${styles.price}`}>
          {currencyFormat(price)}
        </p>

        <p
          className={`text-[14px] ${inter.className} ${styles.colors}`}
        >
          {colors ? (
            colors.map((color) => {
              ...
              return (
                ...
                >
                  <span className={baskerville.className}>
                    {color}
                  </span>
                </button>
              );
        ...
        <div className={styles.sizes}>
          {!sizes ? (
            <p className={`${inter.className} ${styles.size}`}>
              Tamanho único
            </p>
          ) : (
            sizes.map((size, index) => {
              return (
                <button
                  key={index}
                  className={`${
                    size === sizeSelected?.toString() ? styles.selected : ''
                  } ${styles.size}`}
                  onClick={() => {
                    if (sizeSelected?.toString() === size) {
                      setSizeSelected(null);
                      return;
                    }
                    setSizeSelected(Number(size));
                  }}
                >
